...
data.train.sub.tree<-data.train.sub %>% select(-ORIGIN_AIRPORT_STATE, -DESTINATION_AIRPORT_STATE, -DESTINATION_AIRPORT, - ORIGIN_AIRPORT) # removing vars with > 32 factors per function
...
plotcp(rtree.d1)
printcp(rtree.d1)
rtree.d1.prune<-prune.rpart(rtree.d1, cp= 0.0021186442)
plot(as.party(rtree.d1.prune))
```
...
{
fit.rf <- ranger(FLIGHT_DELAY_15~., data=data.train.sub[complete.cases(data.train.sub)], mtry=p, num.trees=250, splitrule="gini", importance="impurity", probability=T)
...
lines(1:31, rf.error.p) # 5 mtries 



delay.ranger<-ranger(FLIGHT_DELAY_15~., data=data.train.sub[complete.cases(data.train.sub)], mtry=5, num.trees=250, splitrule="gini", importance="impurity", probability=F, classification = T)
delay.ranger.p<-ranger(FLIGHT_DELAY_15~., data=data.train.sub[complete.cases(data.train.sub)], mtry=5, num.trees=250, splitrule="gini", importance="impurity", probability=T, classification = F)
...
delay.rf<-randomForest(FLIGHT_DELAY_15~., data=data.train.sub.tree[complete.cases(data.train.sub.tree)], mtry=5, num.trees=250)
...
fit.ranger.delay.predict.y<-predict(delay.ranger, data =data.val, predict.all = F)
  fit.ranger.delay.predic<-predict(delay.ranger.p, data=data.val, type="response", probability=T)
    fit.ranger.delay.predic$predictions<-c(fit.ranger.delay.predic$predictions)
    fit.ranger.delay.predic$predictions
    # testing error
  ranger.test.err<-mean(data.val$FLIGHT_DELAY_15 != fit.ranger.delay.predict.y$predictions)
      ranger.test.err
      ranger.roc<-roc(data.val$FLIGHT_DELAY_15, fit.ranger.delay.predic$predictions[,2], plot=T)
      plot(ranger.roc)
      ranger.auc<-auc<-ranger.roc
    delay.ranger.auc<-auc(ranger.roc)




Delay.Errors<-data.frame(delay.rf$err.rate[250])
  colnames(Delay.Errors)<-"training OOB error"
...
Delay.Errors.ranger<-data.frame(delay.ranger.p$prediction.error)
    colnames(Delay.Errors.ranger)<-"training OOB error"
...
rbind(Delay.Errors, Delay.Errors.ranger)
names(Delay.Errors)
names(Delay.Errors.ranger)
