...
plotcp(rtree.d1)
printcp(rtree.d1)
rtree.d1.prune<-prune.rpart(rtree.d1, cp= 0.00154083       )
plot(as.party(rtree.d1.prune), gp=gpar(fontsize=7))
```
...
rf.error.p <- 1:31 # set up a vector of length 31
for (p in 1:31) # repeat the following code inside { } 31 times
{
...
data.train.sub.lasso<-data.train.sub %>% drop_na()
...
coef.1se<-coef(fit.lasso, s="lambda.1se") # month, airline, 
